...
class TestLoadConfig:
    ...
    def test_loads_defaults_when_no_env_overrides(self) -> None:
        config = load_config()
        assert config.broker_url == "mqtt://localhost:1883"
        assert config.topic_prefix == "clocks/commands"
        assert config.qos == 1

    def test_overrides_via_dict(self) -> None:
    ...
    def test_env_var_non_empty_wins(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestReadDotenv:
    ...
    def test_ignores_unrelated_env_vars(self, tmp_path: Path) -> None:
        env_file = tmp_path / ".env"
