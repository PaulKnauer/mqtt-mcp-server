...
def check_brightness_level(level: int) -> int:
    ...
    Raises:
        BrightnessOutOfRangeError: if level is outside 0-100.

    """
    if not (0 <= level <= 100):
        raise BrightnessOutOfRangeError(level)
    return level
...
def check_message(message: str) -> str:
    ...
    Raises:
        EmptyMessageError: if message is empty or whitespace only.
...
def check_duration(duration_seconds: int) -> int:
    ...
    Raises:
        DurationOutOfRangeError: if duration is outside 1-3600.

    """
    if not (1 <= duration_seconds <= 3600):
        raise DurationOutOfRangeError(duration_seconds)
...
def validate_device_id(device_id: str) -> str:
    ...
    Raises:
        InvalidDeviceIdError: if device_id doesn't match ^[a-zA-Z0-9_-]{1,64}$.
