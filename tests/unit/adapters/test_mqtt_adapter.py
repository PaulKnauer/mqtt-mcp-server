...
class TestMqttAdapterConnect:
    ...
    def test_successful_connect_parses_url(self) -> None:
        adapter = MqttAdapter()
    ...
    def test_successful_connect_default_port(self) -> None:
        adapter = MqttAdapter()
        with patch("mqtt_mcp.adapters.mqtt_adapter.mqtt.Client") as mock_client_cls:
            mock_client_cls.return_value = self._make_mock_client()
    ...
    def test_connect_with_username_password(self) -> None:
        adapter = MqttAdapter()
        with patch("mqtt_mcp.adapters.mqtt_adapter.mqtt.Client") as mock_client_cls:
    ...
    def test_connect_mqtts_enables_tls(self) -> None:
        adapter = MqttAdapter()
        with patch("mqtt_mcp.adapters.mqtt_adapter.mqtt.Client") as mock_client_cls:
            mock_client = self._make_mock_client()
            mock_client_cls.return_value = mock_client
...
class TestMqttAdapterPublish:
    ...
    def test_publish_failure_raises_dispatch_error(self) -> None:
        from paho.mqtt.enums import MQTTErrorCode
